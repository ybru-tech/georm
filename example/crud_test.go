...
func equalTableWithGeometries(t *testing.T, expect, actual TableWithGeometries) {
	t.Helper()
...
type TableWithGeometries struct {
	gorm.Model
	Point              georm.Point
...
func TestCRUDTableWithGeometries(t *testing.T) {
	migrator := db.Migrator()

	err := migrator.AutoMigrate(&TableWithGeometries{})
	require.NoError(t, err)

	var (
		object TableWithGeometries

		objectForCreate = TableWithGeometries{
			Point:              _point1,
		...
		objectForUpdate = TableWithGeometries{
			Point:              _point2,
	...
	err = migrator.DropTable(&TableWithGeometries{})
	require.NoError(t, err)
}
